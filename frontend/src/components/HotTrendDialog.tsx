    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-[900px] h-[600px] fin-panel border fin-divider rounded-xl shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          {/* 右侧热点列表 */}
          <HotItemList key={selectedPlatform} result={currentResult} loading={loading} />
        </div>
      </div>
    </div>
  );
...
const DialogHeader: React.FC<{
  ...
  <div className="flex items-center justify-between px-5 py-4 border-b fin-divider shrink-0">
    <div className="flex items-center gap-3">
      ...
      <div>
        <h2 className="text-lg font-bold text-white">全网热点</h2>
        <p className="text-xs text-slate-400">实时舆情监控</p>
      </div>
    </div>
...
}> = ({ results, selectedPlatform, onSelect }) => (
      ...
      >
        <div className="flex-1 min-w-0">
          ...
          {result.error ? (
            ...
            <div className="text-xs text-slate-500">{result.items?.length || 0} 条</div>
          )}
        </div>
      </button>
    ))}
...
const HotItemList: React.FC<{
  ...
  return (
    <div className="flex-1 overflow-y-auto fin-scrollbar p-3">
      {items.map((item, idx) => (
        <HotItemRow key={item.id || idx} item={item} />
      ))}
    </div>
  );
};
...
const HotItemRow: React.FC<{ item: hottrend.HotItem }> = ({ item }) => {
  ...
  return (
    <div
      onClick={handleClick}
      className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-slate-700/50 cursor-pointer transition-colors group"
    >
      ...
      {item.extra && (
        <span className="text-xs text-slate-500 shrink-0">{item.extra}</span>
      )}
